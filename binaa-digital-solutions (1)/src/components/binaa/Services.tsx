...
export function Services() {
  ...
  const webTags = lang === "ar"
    ? ["React / Next.js", "متجاوب", "آمن"]
    : ["React / Next.js", "Responsive", "Secure"];
  ...
  return (
    <section id="services" className="relative mx-auto max-w-6xl px-5 py-16 sm:px-6 sm:py-20 text-white">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-6 lg:h-[34rem]">
        ...
        <article className="relative overflow-hidden rounded-3xl bg-[#14213d]/80 backdrop-blur-xl border border-[#7df56e]/30 p-7 hover:border-[#7df56e]/70 transition-all duration-500 sm:col-span-2 flex flex-col justify-between shadow-2xl">
          <div>
            ...
            <h3 className="mt-5 text-2xl font-bold font-display">
              {t.services.items[1]?.title || (lang === "ar" ? "حلول الذكاء الاصطناعي والشات بوت" : "AI Solutions & Chatbots")}
            </h3>
            <p className="mt-3 text-sm leading-relaxed text-zinc-300">
              {t.services.items[1]?.desc || (lang === "ar" ? "مساعدين أذكياء يردّون على عملائك، يرشّحون المنتجات، ويعملون ٢٤ ساعة دون توقف." : "Integrating generative AI models for smart conversational bots and automated workflows.")}
            </p>
          </div>
        </article>
        ...
        <article className="relative overflow-hidden rounded-3xl bg-[#14213d]/60 backdrop-blur-xl border border-white/10 p-7 hover:border-[#7df56e]/50 transition-all duration-500 sm:col-span-3 flex flex-col justify-between shadow-xl">
          <div>
            ...
            <h3 className="mt-5 text-2xl font-bold sm:text-3xl font-display">
              {t.services.items[3]?.title || (lang === "ar" ? "تصميم متاجر سلة" : "Salla Stores Design")}
            </h3>
            <p className="mt-3 text-sm leading-relaxed text-zinc-300">
              {t.services.items[3]?.desc || (lang === "ar" ? "ثيمات وتخصيصات لمتجرك على منصة سلة برفع واضح لمعدل الشراء." : "Custom designing professional Salla store interfaces to boost conversions.")}
            </p>
          </div>
        </article>
      </div>
